...
class Feed(models.Model):
    ...
    def feed_check(self, force=False, read=False, logfile=None):
        ...
        * it is due for an update in the next ``MINIMUM_INTERVAL`` minutes
        ...
        self.entries.filter(expires__lte=datetime.datetime.utcnow().replace(tzinfo=utc)).delete()
    ...
    def _update_entries(self, entries, read):
        ...
        for raw_entry in entries:
            ...
            if entry.guid:
                ...
                query = {
                    'title': entry.title,
                    'date': entry.date.replace(tzinfo=timezone.utc),
                }
            else:
            ...
            try:
                ...
                # Existing entry
                if entry.date.replace(tzinfo=timezone.utc) is not None and \
                                entry.date.replace(tzinfo=timezone.utc) > existing.date:
